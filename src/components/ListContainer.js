 ...
 dragTaskHandler(dragId, ctnrId, title)
  ...
  return (
    <div className="listctnr" >
      <div className="titlebar" style={getStyle(theme.color).headerStyle}>
        {title}
        <span style={{padding: '7px'}}>{data.length}</span>
      </div>
      <div className = "ctnr" style ={getStyle(theme.color).ctnrStyle}
      droppable = 'true'
      onDrop= { onDrop}
      ...
      >
        ...
        {data.length > 0 ? data.map(el => <TaskItem key= {el.id} 
        id={el.id} task={el.task} status={el.status}
          taskHandlerL={(id) => taskHandler(id, title)}  title = {title}/>)
